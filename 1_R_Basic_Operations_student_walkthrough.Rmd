...
```{r}
is.integer(a) # false its a boolean
is.numeric(a) # true!
```
...
first_name <- "Joe"
last_name <- "Smith"
full_name <- paste(first_name,last_name) # comma automatically adds a space
full_name
```
...
```{r}
# 's' is probably from the parent language 'S'
# have datatypes that HAVE to go with these symbols
# a lot of this language was very mathy people having fun
# double precision values are basically floats from python

# sprintf(string, 1st symbol replacement, 2nd symbol replacement etc.)
...
# easy trick to see what something is as in scientific notation!
```
...
# inclusive stop (goes one further)
```
...
```{r}
# useful trick for tidying data
# Hadley Wichkam quote on tidying data

sub("much", "little", "How much wood could a woodchuck chuck?")
# replaced first word with second

# help(sub) there are regex behind the scenes!
```
...
```{r}
x & y # both would have to be true to be true
```
...
1.  Create two strings with your first and last name, respectively. Then concatenate them, and print the new value as "My name is \<your newly created concatenated variable will go here\>"
    ...
    full_name
    glue("My name is {full_name}")
    ```

2.  What values can a BOOLEAN contain?

TRUE or FALSE, 0 or 1, Yes or No
